...
이 문서는 WeKnora 프로젝트에 새로운 벡터 데이터베이스 지원을 추가하는 전체 가이드를 제공합니다. 표준화된 인터페이스를 구현하고 구조화된 프로세스를 따르면 개발자는 사용자 지정 벡터 데이터베이스를 효율적으로 통합할 수 있습니다.
...
`RetrieveEngineService` 인터페이스를 구현하는 서비스를 생성하여 인덱스 생성 및 관리 비즈니스 로직을 처리합니다.
...
환경 설정에 새 데이터베이스의 필수 연결 매개변수를 추가합니다.
...
internal/container/container.go 파일의 initRetrieveEngineRegistry 함수에 새 데이터베이스 초기화 및 등록 로직을 추가합니다.
...
기존 PostgreSQL 및 Elasticsearch 구현을 개발 템플릿으로 참고하는 것을 권장합니다. 해당 구현은 다음 디렉토리에 있습니다:
...
위 단계들을 따르고 기존 구현을 참고하면, 새로운 벡터 데이터베이스를 WeKnora 시스템에 성공적으로 통합하여 벡터 검색 기능을 확장할 수 있습니다.
