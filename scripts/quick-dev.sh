#!/bin/bash
# 빠른 개발 환경 시작 스크립트 (One-Click)
# 이 스크립트는 하나의 터미널에서 필요한 모든 서비스를 시작합니다.
...
if [ "$start_backend" = "y" ] || [ "$start_backend" = "Y" ]; then
    ...
    log_info "나중에 새 터미널에서 실행하세요: make dev-app 또는 ./scripts/dev.sh app"
fi
...
if [ "$start_frontend" = "y" ] || [ "$start_frontend" = "Y" ]; then
    ...
    log_info "나중에 새 터미널에서 실행하세요: make dev-frontend 또는 ./scripts/dev.sh frontend"
fi
...
echo ""
log_success "개발 환경 준비 완료. 코딩을 시작하세요!"
echo ""