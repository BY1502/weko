...
# 서비스 준비 대기
log_info "서비스 시작 완료 대기..."
sleep 5

# 2. 백엔드 시작 여부 확인
echo ""
log_info "단계 2/3: 백엔드 앱 시작"
printf "%b" "${YELLOW}현재 터미널에서 백엔드를 시작할까요? (y/N): ${NC}"
...
if [ "$start_backend" = "y" ] || [ "$start_backend" = "Y" ]; then
    ...
    log_success "백엔드가 백그라운드에서 실행 중 (PID: $BACKEND_PID)"
...
# 3. 프론트엔드 시작 여부 확인
echo ""
log_info "단계 3/3: 프론트엔드 앱 시작"
printf "%b" "${YELLOW}현재 터미널에서 프론트엔드를 시작할까요? (y/N): ${NC}"
read -r start_frontend

if [ "$start_frontend" = "y" ] || [ "$start_frontend" = "Y" ]; then
    log_info "프론트엔드를 시작합니다..."
    # 백그라운드에서 프론트엔드 시작
    ...
    log_success "프론트엔드가 백그라운드에서 실행 중 (PID: $FRONTEND_PID)"
