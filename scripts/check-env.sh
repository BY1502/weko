...
if [ -f ".env" ]; then
    ...
    echo "  2. .env 파일을 열어 필요한 환경 변수를 채웁니다"
    exit 1
fi
...
if [ -n "$INIT_LLM_MODEL_NAME" ]; then
    ...
    log_warning "INIT_LLM_MODEL_NAME 이 설정되지 않았습니다(선택)"
fi

if [ -n "$INIT_EMBEDDING_MODEL_NAME" ]; then
    log_success "INIT_EMBEDDING_MODEL_NAME = $INIT_EMBEDDING_MODEL_NAME"
else
    log_warning "INIT_EMBEDDING_MODEL_NAME 이 설정되지 않았습니다(선택)"
fi

# Go 환경 확인
echo ""
log_info "Go 환경을 확인합니다..."
...
if [ $errors -eq 0 ]; then
    ...
    echo "  - DB_DRIVER 가 'postgres' 혹은 'mysql' 인지 확인하세요."
