...
if command -v air &> /dev/null; then
    ...
    log_warning "Air 미설치 (핫 리로드용, 선택 사항)"
    log_info "설치 명령어: go install github.com/cosmtrek/air@latest"
fi

# npm 검사
echo ""
log_info "Node.js 환경 검사 중..."
...
if command -v docker &> /dev/null; then
    ...
    if docker info &> /dev/null; then
        ...
        log_error "Docker 서비스 실행 안 됨"
        errors=$((errors + 1))
    fi
else
    log_error "Docker 미설치"
    errors=$((errors + 1))
fi
...
printf "%b\n" "${GREEN}========================================${NC}"
if [ $errors -eq 0 ]; then
    log_success "모든 검사 통과! 환경 설정이 정상입니다."
    echo ""
    log_info "다음 단계:"
    ...
    log_error "$errors 개의 문제가 발견되었습니다. 수정 후 다시 시도해 주세요."
    echo ""
    log_info "자주 발생하는 문제:"
    echo "  - .env 파일이 없다면 .env.example을 복사하세요."
