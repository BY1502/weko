...
# 프로젝트 루트 디렉토리 가져오기 (스크립트가 있는 폴더의 상위 폴더)
...
show_help() {
    ...
    echo "  -p, --app      앱(백엔드) 이미지만 빌드"
    echo "  -d, --docreader 문서 리더(DocReader) 이미지만 빌드"
...
check_platform() {
    ...
    if [ "$(uname -m)" = "x86_64" ]; then
        ...
        log_warning "알 수 없는 플랫폼 유형: $(uname -m), 기본 플랫폼 linux/amd64를 사용합니다."
        export PLATFORM="linux/amd64"
        export TARGETARCH="amd64"
    fi
...
# 앱 이미지 빌드
build_app_image() {
    log_info "앱 이미지 빌드 중 (weknora-app)..."
