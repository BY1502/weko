```bash
#!/bin/bash
# 개발 환경 시작 스크립트 - 인프라만 시작하며, app과 frontend는 로컬에서 수동으로 실행해야 합니다.
...
check_docker() {
    ...
    if ! detect_compose_cmd; then
        log_error "Docker Compose가 감지되지 않았습니다."
        return 1
    fi
...
start_services() {
    ...
    # .env 파일 확인
    if [ ! -f ".env" ]; then
        log_error ".env 파일이 없습니다. 먼저 생성해 주세요."
        return 1
    fi
...
start_app() {
    ...
    # 환경 변수 로드 (set -a 사용하여 모든 변수 export)
    if [ -f ".env" ]; then
        ...
        log_error ".env 파일이 없습니다. 설정 파일을 먼저 생성하세요."
        return 1
    fi
    ...
    log_info "환경 변수 설정 완료, 앱 시작..."
    log_info "DB 주소: $DB_HOST:${DB_PORT:-5432}"
    
    # Air(핫 리로드 도구) 설치 여부 확인
    if command -v air &> /dev/null; then
        log_success "Air 감지됨, 핫 리로드 모드로 시작..."
        log_info "Go 코드 수정 시 자동으로 재컴파일 및 재시작됩니다."
        air
    else
        log_info "Air 미감지, 일반 모드로 시작"
