#!/bin/bash
# 개발 환경 시작 스크립트 - 인프라만 띄우고 app/frontend는 로컬에서 직접 실행
...
show_help() {
    ...
    echo "  app        백엔드 앱 시작(로컬)"
    echo "  frontend   프론트엔드 개발 서버 시작(로컬)"
    echo "  help       이 도움말 표시"
    echo ""
    echo "선택 Profile(start 명령용):"
    echo "  --minio    MinIO 객체 스토리지"
    echo "  --qdrant   Qdrant 벡터 DB"
    echo "  --neo4j    Neo4j 그래프 DB"
    echo "  --jaeger   Jaeger 트레이싱"
    echo "  --full     모든 선택 서비스"
    echo ""
    echo "예시:"
    echo "  $0 start                    # 기본 서비스 시작"
    echo "  $0 start --qdrant           # 기본 + Qdrant"
    echo "  $0 start --qdrant --jaeger  # 기본 + Qdrant + Jaeger"
    echo "  $0 start --full             # 모든 서비스 시작"
    echo "  $0 app                      # 다른 터미널에서 백엔드 실행"
    echo "  $0 frontend                 # 다른 터미널에서 프론트엔드 실행"
}
...
check_docker() {
    ...
    if ! detect_compose_cmd; then
        log_error "Docker Compose를 찾지 못했습니다"
        return 1
    fi
...
# 백엔드 앱 시작(로컬)
start_app() {
    log_info "백엔드 앱을 시작합니다(로컬 개발 모드)..."
    ...
    # 로컬 개발 환경 변수 설정(컨테이너 주소 override)
    ...
    # 필수 환경 변수 확인
    if [ -z "$DB_DRIVER" ]; then
        log_error "DB_DRIVER 환경 변수가 설정되지 않았습니다. .env를 확인하세요."
        return 1
    fi
    
    log_info "환경 변수가 설정되었습니다. 앱을 시작합니다..."
    log_info "DB 주소: $DB_HOST:${DB_PORT:-5432}"
    
    # Air(핫 리로드) 설치 여부
    if command -v air &> /dev/null; then
        log_success "Air 감지, 핫 리로드 모드로 시작..."
        log_info "Go 코드를 수정하면 자동으로 재컴파일/재시작됩니다"
        air
    else
        log_info "Air가 없어 일반 모드로 시작합니다"
        log_warning "참고: Air를 설치하면 코드 수정 시 자동 재시작됩니다"
...
start_frontend() {
    ...
    if [ ! -d "node_modules" ]; then
        log_warning "node_modules 가 없어 의존성을 설치합니다..."
        npm install
    fi
    
    log_info "Vite 개발 서버를 시작합니다..."
    log_info "프론트엔드 주소: http://localhost:5173"
