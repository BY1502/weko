...
#### 예시: chat_completion.go에서 이벤트 추가
...
1. **핵심 경로에서 동기 이벤트 버스 사용 자제**: 모니터링/로그 등 비즈니스에 영향 없는 경우 비동기 사용
2. **미들웨어는 필요한 곳에만**: 불필요한 오버헤드 최소화
3. **이벤트 데이터 크기 관리**: 특히 비동기 모드에서 대량 데이터 전달을 피하기
4. **전용 리스너 사용**: 한 리스너에 여러 기능을 넣지 말고 단일 책임을 유지
