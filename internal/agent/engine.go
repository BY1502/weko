...
func (e *AgentEngine) executeLoop(
	...
	if !state.IsComplete {
		...
		if err := e.streamFinalAnswerToEventBus(ctx, query, state, sessionID); err != nil {
			...
			state.FinalAnswer = "죄송합니다. 전체 답변을 생성할 수 없습니다."
		}
		state.IsComplete = true
	}
...
func (e *AgentEngine) streamReflectionToEventBus(
	...
	reflectionPrompt := fmt.Sprintf(`방금 호출한 도구 %s의 결과를 평가하고 다음 행동을 결정하세요.

도구 반환: %s

생각:
1. 결과가 요구사항을 충족합니까?
2. 다음에 무엇을 해야 합니까?`, toolName, result)
...
func (e *AgentEngine) streamFinalAnswerToEventBus(
	...
	finalPrompt := fmt.Sprintf(`위 도구 호출 결과를 바탕으로 사용자 질문에 대한 완전한 답변을 작성하세요。

사용자 질문: %s

요구사항:
1. 실제 검색된 내용을 기반으로 답변하세요
2. 정보 출처를 명확히 표기하세요(chunk_id, 문서명)
3. 답변을 구조화하여 작성하세요
4. 정보가 부족하면 솔직히 명시하세요

이제 최종 답변을 생성하세요:`, query)
